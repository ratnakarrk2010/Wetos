...
namespace WetosMVC.Controllers
{
    ...
    public class DashboardController : BaseController  // Updated on Rajas on 30 MARCH 2017
    {
        ...
        public ActionResult HRDashboard()
        {
            ...
            //              select y).Count();
            int Total1 = (from y in WetosDB.DailyTransactions
                          where ((y.TranDate >= DateTime.Today) && (y.Status != "AAAA"))
                          select y).Count();
            if (Total1 > 0)
            {
            ...
            int FemalePresentCount = (from y in WetosDB.DailyTransactions
                                      join z in WetosDB.Employees on y.EmployeeId equals z.EmployeeId
                                      where ((z.Gender.ToUpper().Trim() == "FEMALE" || z.Gender.Trim() == "F") && (y.TranDate >= DateTime.Today) && (y.Status != "AAAA")
                                      )
                                      select y).Count();
            ...
            int MalePresentCount = (from y in WetosDB.DailyTransactions
                                    join z in WetosDB.Employees on y.EmployeeId equals z.EmployeeId
                                    where ((z.Gender.ToUpper().Trim() == "MALE" || z.Gender.Trim() == "M") && (y.TranDate >= DateTime.Today) && (!y.Status.Contains("A")))
                                    select y).Count();
